...
parser.add_argument(
    "line_ref", 
    type=str, 
    help="path to xsuite line to take reference twiss from"
)
parser.add_argument(
    "colldb", 
    type=str, 
...
parser.add_argument(
    "ip_orbit", 
    type=int, 
    help="which IP to modify the local orbit in (1 or 5)"
)
parser.add_argument(
    "ip_orbit_knob_strength", 
    type=float, 
    help="strength of IP long crossing bump knob"
)
parser.add_argument(
    "ip_orbit_plane", 
    type=str, 
    help="which plane to modify the local orbit in at the IP (H, V or HV)"
)
parser.add_argument(
    "num_turns", 
    type=int, 
...
parser.add_argument(
    "start_batch", 
    type=int, 
    help="batch number to start tracking from"
)
...
def track_single_batch(
    ...
    if ip_orbit == 1:
        if ip_orbit_plane == "H":
            line.vars["on_x1hl"] = 94 * ip_orbit_knob_strength
        elif ip_orbit_plane == "V":
            line.vars["on_x1vl"] = 71 * ip_orbit_knob_strength
        elif ip_orbit_plane == "HV":
            line.vars["on_x1hl"] = 94 * ip_orbit_knob_strength
            line.vars["on_x1vl"] = 71 * ip_orbit_knob_strength
        else:
            raise ValueError("Plane requested for local orbit change in IP is invalid!")
    elif ip_orbit == 5:
        if ip_orbit_plane == "H":
            line.vars["on_x5hl"] = 94 * ip_orbit_knob_strength
        elif ip_orbit_plane == "V":
            line.vars["on_x5vl"] = 71 * ip_orbit_knob_strength
        elif ip_orbit_plane == "HV":
            line.vars["on_x5hl"] = 94 * ip_orbit_knob_strength
            line.vars["on_x5vl"] = 71 * ip_orbit_knob_strength
        else:
        ...
        raise ValueError("IP requested for local orbit change is invalid!")

    line_ref = xt.load(line_ref_path)
    line_ref.vars[f"i_mo.b{beam}"] = i_mo
    ...
    tw = line.twiss()
    tw_ref = line.twiss()
    line.xcoll.collimators.assign_optics(twiss=tw_ref)
    ...
    for name in failing_names:
        for i in range(7):
            line.element_refs[name].knl[i] += line.env.vars[f"cliq_Kn{i}"] / Brho * line.env[name].length
            line.element_refs[name].ksl[i] += line.env.vars[f"cliq_Ks{i}"] / Brho * line.env[name].length
    ...
    with open(f'part_fin_batch{batch}_seed{seed}_{cliq_case}_{halo_model}_{dist_mode}_imo_{int(i_mo)}_orbit{ip_orbit_plane}{int(ip_orbit_knob_strength*100)}.pkl', 'wb') as file:
...
def main():
    ...
    i_mo = args.i_mo
    ip_orbit = args.ip_orbit
    ip_orbit_knob_strength = args.ip_orbit_knob_strength
    ip_orbit_plane = args.ip_orbit_plane
    ...
    Run 10 batches
    '''
    for batch in range(start_batch, 10):
