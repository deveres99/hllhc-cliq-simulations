...
def main():
    ...
    for cliq_case in cliq_cases:
        ...
        ax.step(
            ...
            color=colors[cliq_case[:-1]], 
            # marker=markers[cliq_case[-1]], 
            # markersize=markersize,
            linestyle=linestyles[cliq_case[-1]], 
            where='mid', 
            label=cliq_case
        )
    ...
    ax.legend(loc="lower right", ncol=6, fontsize=fs-6)
    ...
    if save:
        fig.savefig(result_path / f"losses_vs_time_log_{halo_model}_{dist_mode}_imo_{int(i_mo)}.png", dpi=150, format='png', bbox_inches='tight')
