...
dist_modes = ['2D', '4D']
imo_vals = [0, -100, -200, -300, -400, -500]
...
def plot_1MJ_exceeded(
    ...
    for x, y in zip(imo_vals, y_s_BLM * 1e6):
        ax.plot(
            [x-10, x+10], 
            [y, y], 
    ...
    for x, y_blm, y in zip(imo_vals, y_s_BLM * 1e6, y_s * 1e6):
        ax.plot(
            [x, x], 
            [y_blm, y], 
            color=colors[halo_model][dist_mode], 
...
def plot_max_collimator_losses(ax, y_primary, y_total, halo_model, dist_mode):
    ...
    for x, y_p, y_t in zip(imo_vals, y_primary, y_total):
        ax.plot(
            [x, x], 
            [y_p, y_t], 
            color=colors[halo_model][dist_mode], 
...
def main():
    ...
    if halo_model == "all" and dist_mode == "all":
        with open(result_path / f"mean_halo_2D_imo{int(imo_vals[0])}/observables_{cliq_case}.pkl", "rb") as file:
            obs_mean_2D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"mean_halo_2D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
        ...
        with open(result_path / f"mean_halo_4D_imo{int(imo_vals[0])}/observables_{cliq_case}.pkl", "rb") as file:
            obs_mean_4D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"mean_halo_4D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_mean_4D.update({
        ...
        with open(result_path / f"cons_halo_2D_imo{int(imo_vals[0])}/observables_{cliq_case}.pkl", "rb") as file:
            obs_cons_2D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"cons_halo_2D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_cons_2D.update({
        ...
        with open(result_path / f"cons_halo_4D_imo{int(imo_vals[0])}/observables_{cliq_case}.pkl", "rb") as file:
            obs_cons_4D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"cons_halo_4D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_cons_4D.update({
        ...
        idxs_primary_PDSU_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_PDSU_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_primary_PDSU_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_PDSU_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        idxs_primary_BLM_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_BLM_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_primary_BLM_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_BLM_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        idxs_primary_1MJ_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_1MJ_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_primary_1MJ_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_1MJ_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        idxs_total_PDSU_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_total_PDSU_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_total_PDSU_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_total_PDSU_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        idxs_total_BLM_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_total_BLM_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_total_BLM_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_total_BLM_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        idxs_total_1MJ_mean_2D = np.full(len(imo_vals), np.nan)
        idxs_total_1MJ_mean_4D = np.full(len(imo_vals), np.nan)
        idxs_total_1MJ_cons_2D = np.full(len(imo_vals), np.nan)
        idxs_total_1MJ_cons_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        with open(result_path / f"{halo_model}_halo_2D_imo{int(imo_vals[0])}/observables.pkl", "rb") as file:
            obs_2D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"{halo_model}_halo_2D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_2D.update({
        ...
        with open(result_path / f"{halo_model}_halo_4D_imo{int(imo_vals[0])}/observables.pkl", "rb") as file:
            obs_4D = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"{halo_model}_halo_4D_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_4D.update({
        ...
        if "" in names_primary_PDSU:
            names_primary_PDSU = np.delete(names_primary_PDSU, np.where(names_primary_PDSU == "")[0][0])
        idxs_primary_PDSU_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_PDSU_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_BLM:
            names_primary_BLM = np.delete(names_primary_BLM, np.where(names_primary_BLM == "")[0][0])
        idxs_primary_BLM_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_BLM_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_1MJ:
            names_primary_1MJ = np.delete(names_primary_1MJ, np.where(names_primary_1MJ == "")[0][0])
        idxs_primary_1MJ_2D = np.full(len(imo_vals), np.nan)
        idxs_primary_1MJ_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_PDSU:
            names_total_PDSU = np.delete(names_total_PDSU, np.where(names_total_PDSU == "")[0][0])
        idxs_total_PDSU_2D = np.full(len(imo_vals), np.nan)
        idxs_total_PDSU_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_BLM:
            names_total_BLM = np.delete(names_total_BLM, np.where(names_total_BLM == "")[0][0])
        idxs_total_BLM_2D = np.full(len(imo_vals), np.nan)
        idxs_total_BLM_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_1MJ:
            names_total_1MJ = np.delete(names_total_1MJ, np.where(names_total_1MJ == "")[0][0])
        idxs_total_1MJ_2D = np.full(len(imo_vals), np.nan)
        idxs_total_1MJ_4D = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        with open(result_path / f"mean_halo_{dist_mode}_imo{int(imo_vals[0])}/observables.pkl", "rb") as file:
            obs_mean = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"mean_halo_{dist_mode}_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_mean.update({
        ...
        with open(result_path / f"cons_halo_{dist_mode}_imo{int(imo_vals[0])}/observables.pkl", "rb") as file:
            obs_cons = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"cons_halo_{dist_mode}_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs_cons.update({
        ...
        if "" in names_primary_PDSU:
            names_primary_PDSU = np.delete(names_primary_PDSU, np.where(names_primary_PDSU == "")[0][0])
        idxs_primary_PDSU_mean = np.full(len(imo_vals), np.nan)
        idxs_primary_PDSU_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_BLM:
            names_primary_BLM = np.delete(names_primary_BLM, np.where(names_primary_BLM == "")[0][0])
        idxs_primary_BLM_mean = np.full(len(imo_vals), np.nan)
        idxs_primary_BLM_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_1MJ:
            names_primary_1MJ = np.delete(names_primary_1MJ, np.where(names_primary_1MJ == "")[0][0])
        idxs_primary_1MJ_mean = np.full(len(imo_vals), np.nan)
        idxs_primary_1MJ_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_PDSU:
            names_total_PDSU = np.delete(names_total_PDSU, np.where(names_total_PDSU == "")[0][0])
        idxs_total_PDSU_mean = np.full(len(imo_vals), np.nan)
        idxs_total_PDSU_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_BLM:
            names_total_BLM = np.delete(names_total_BLM, np.where(names_total_BLM == "")[0][0])
        idxs_total_BLM_mean = np.full(len(imo_vals), np.nan)
        idxs_total_BLM_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_1MJ:
            names_total_1MJ = np.delete(names_total_1MJ, np.where(names_total_1MJ == "")[0][0])
        idxs_total_1MJ_mean = np.full(len(imo_vals), np.nan)
        idxs_total_1MJ_cons = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        with open(result_path / f"{halo_model}_halo_{dist_mode}_imo{int(imo_vals[0])}/observables.pkl", "rb") as file:
            obs = pickle.load(file)
        for i in imo_vals[1:]:
            with open(result_path / f"{halo_model}_halo_{dist_mode}_imo{int(i)}/observables_{cliq_case}.pkl", "rb") as file:
                tmp = pickle.load(file)
                obs.update({
        ...
        if "" in names_primary_PDSU:
            names_primary_PDSU = np.delete(names_primary_PDSU, np.where(names_primary_PDSU == "")[0][0])
        idxs_primary_PDSU = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_BLM:
            names_primary_BLM = np.delete(names_primary_BLM, np.where(names_primary_BLM == "")[0][0])
        idxs_primary_BLM = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_primary_1MJ:
            names_primary_1MJ = np.delete(names_primary_1MJ, np.where(names_primary_1MJ == "")[0][0])
        idxs_primary_1MJ = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_PDSU:
            names_total_PDSU = np.delete(names_total_PDSU, np.where(names_total_PDSU == "")[0][0])
        idxs_total_PDSU = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_BLM:
            names_total_BLM = np.delete(names_total_BLM, np.where(names_total_BLM == "")[0][0])
        idxs_total_BLM = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
        ...
        if "" in names_total_1MJ:
            names_total_1MJ = np.delete(names_total_1MJ, np.where(names_total_1MJ == "")[0][0])
        idxs_total_1MJ = np.full(len(imo_vals), np.nan)
        for i in range(len(imo_vals)):
    ...
    '''
    # Time/turn of 1 MJ exceeded
    ax1.set_xticks(imo_vals, labels=imo_vals)
    ax1.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    ax1_a.set_xticks(imo_vals, labels=imo_vals)
    ax1_a.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    ax1_b.set_xticks(imo_vals, labels=imo_vals)
    ax1_b.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Total losses at time of PDSU dump
    ax2.set_xticks(imo_vals, labels=imo_vals)
    ax2.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Total losses at time of BLM dump
    ax3.set_xticks(imo_vals, labels=imo_vals)
    ax3.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Maximum losses (primary and total) on a single collimator at time of PDSU dump
    ax4.set_xticks(imo_vals, labels=imo_vals)
    ax4.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Maximum losses (primary and total) on a single collimator at time of BLM dump
    ax5.set_xticks(imo_vals, labels=imo_vals)
    ax5.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Maximum losses (primary and total) on a single collimator when 1 MJ exceeded
    ax6.set_xticks(imo_vals, labels=imo_vals)
    ax6.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum primary losses at time of PDSU dump
    ax7.set_xticks(imo_vals, labels=imo_vals)
    ax7.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum primary losses at time of BLM dump
    ax8.set_xticks(imo_vals, labels=imo_vals)
    ax8.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum primary losses when 1 MJ exceeded
    ax9.set_xticks(imo_vals, labels=imo_vals)
    ax9.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum total losses at time of PDSU dump
    ax10.set_xticks(imo_vals, labels=imo_vals)
    ax10.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum total losses at time of BLM dump
    ax11.set_xticks(imo_vals, labels=imo_vals)
    ax11.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
    ...
    # Name of collimator with maximum total losses when 1 MJ exceeded
    ax12.set_xticks(imo_vals, labels=imo_vals)
    ax12.set_xlabel(r"$I_\text{MO}$ [A]", fontsize=fs)
